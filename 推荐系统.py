...
users = {"Angelica": {"Blues Traveler": 3.5, "Broken Bells": 2.0, "Norah Jones": 4.5, "Phoenix": 5.0,
         ...
         "yang": {"Blues Traveler": 3.0, "Norah Jones": 5.0, "Phoenix": 4.0, "Slightly Stoopid": 2.5,
                      "The Strokes": 3.0}
         }
...
if __name__ == "__main__":
    distance = manhattan(users["Angelica"],users["yang"])
    print(distance)
    #  获取相似用户列表
    user_list= computeNearestNeighbor("yang",users)
    print(user_list)

    #  获取推荐歌单
    song_list = recommend("yang",users)
    print(song_list)

    #  皮尔斯系数   用于计算用户相似度
    score = pearson(users["yang"],users["Angelica"])
    print(score)